...
public class LinkedBlockingDeque<E> extends AbstractQueue<E>
	...
	public E peekFirst() {
		...
		lock.lock();
		try {
			return (mFirst == null) ? null : mFirst.mItem;
		} finally {
			lock.unlock();
		}
	}
	...
	public E peekLast() {
		...
		lock.lock();
		try {
			return (mLast == null) ? null : mLast.mItem;
		} finally {
			lock.unlock();
		}
	}
	...
	public boolean removeFirstOccurrence(Object o) {
		...
		lock.lock();
		try {
			for (Node<E> p = mFirst; p != null; p = p.mNext) {
				if (o.equals(p.mItem)) {
					unlink(p);
					return true;
				}
	...
	public boolean removeLastOccurrence(Object o) {
		...
		lock.lock();
		try {
			for (Node<E> p = mLast; p != null; p = p.mPrev) {
				if (o.equals(p.mItem)) {
					unlink(p);
					return true;
				}
	...
	public int drainTo(Collection<? super E> c, int maxElements) {
		...
		lock.lock();
		try {
			int n = Math.min(maxElements, mCount);
			for (int i = 0; i < n; i++) {
				c.add(mFirst.mItem);   // In this order, in case add() throws.
				unlinkFirst();
			}
			return n;
		} finally {
	...
	private abstract class AbstractItr implements Iterator<E> {
		...
		AbstractItr() {
			...
			final ReentrantLock lock = LinkedBlockingDeque.this.mLock;
			lock.lock();
			try {
				mNext = firstNode();
